...
void Level::CreateClassic(SceneNode* root, Physics* physics, TextureHolder* texture_holder, sf::FloatRect world_bounds, SoundPlayer& sounds, GameData& data)
{
	...
	std::unique_ptr<Paddle> paddle_one(new Paddle(0, data.GetSelectedCharacter(0), 200, center.y, physics, sounds, stoneWhite));
	dynamic->AttachChild(std::move(paddle_one));

	std::unique_ptr<Paddle> paddle_two(new Paddle(1, data.GetSelectedCharacter(1), world_bounds.size.x - 200, center.y, physics, sounds, stoneWhite));
...
void Level::CreateDiamond(SceneNode* root, Physics* physics, TextureHolder* texture_holder, sf::FloatRect world_bounds, SoundPlayer& sounds, GameData& data)
{
	sf::Texture* wallGrey = &texture_holder->Get(TextureID::kWallGrey);
	sf::Texture* wallRed = &texture_holder->Get(TextureID::kWallRed);
	sf::Texture* stoneWhite = &texture_holder->Get(TextureID::kStoneWhite);
